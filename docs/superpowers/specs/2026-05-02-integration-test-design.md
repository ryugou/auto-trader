...
# Phase 4（実 API、ネットワー���必要）
cargo test -p integration-tests --features external-api
```
...
| ��定バリデーション（disabled 戦略） | `config_disabled_strategy.toml` → スキップ確認 |
...
| Short trailing エ���ジット | `donchian_short_trailing_exit.csv` | 同 Short |
...
| Short エントリ�� | `squeeze_short_entry.csv` | TTM Squeeze ��除 + momentum 下降 |
| Long Chandelier エグジット | `squeeze_long_chandelier_exit.csv` | HH - ATR×3 ブレイク |
| Short Chandelier エグジ��ト | `squeeze_short_chandelier_exit.csv` | LL + ATR×3 |
...
| TP ヒット Long | `tp_hit_long.csv` | TP ��達 |
...
[FAIL] phase3::trade_flow::bb_mean_revert_long_entry
  ...
  DEBUG skipping signal: pair USD_JPY not available on exchange BitflyerCfd for account ��全
...
| `config_unknown_exchange.toml` | ��正 exchange 名 |
