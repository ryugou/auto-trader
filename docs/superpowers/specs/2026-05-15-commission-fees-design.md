...
2. paper (`dry_run=true`) で同じ commission を `commission::estimate(Action::Open|Close, ...)` 関数経由で計算し、`Trade.fees` に累積する。
...
[paper, dry_run=true]
  fill_open  → commission::estimate(Action::Open,  exchange, fill_price, qty) = open_commission
  fill_close → commission::estimate(Action::Close, exchange, fill_price, qty) = close_commission
```
...
| File | 変更 | 内容 |
|------|------|------|
| `crates/core/src/commission.rs` | **新規** | `Action` enum + `estimate(Action, Exchange, Decimal, Decimal) -> Decimal`、Exchange enum を exhaustive match で扱う |
...
/// commission を見積もる対象の約定種別。open/close で別レートを取りたく
/// なった時にここを増やせば、`estimate` の match で全 exchange 対応を強制
/// できる構造。
...
exhaustive match を `_` arm 無しで書くので、新しい Exchange enum variant を追加した時に compile error で気づける (これは leverage validation でも採用した pattern と同じ)。実装中に `estimate_open` / `estimate_close` の 2 関数版でいったん書いたが、両者の body が完全同一だったので simplify cleanup で `Action` enum + 1 関数に統合した (commit `ceb94d2`)。
...
fn estimate_all_exchanges_currently_zero_for_open() {
    ...
    for ex in [Exchange::BitflyerCfd, Exchange::GmoFx, Exchange::Oanda] {
        assert_eq!(estimate(Action::Open, ex, dec!(150), dec!(1)), Decimal::ZERO);
    }
}

#[test]
fn estimate_all_exchanges_currently_zero_for_close() {
    // 同じく 3 件、Action::Close 経由
}
```
