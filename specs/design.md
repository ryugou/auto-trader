...
- PriceEvent を発行
- 取得した candle を `price_candles` テーブルに保存する（バックテスト用データの蓄積）。`upsert_candle()` で重複を避ける
...
- 既にポジションが開いているペア（同一戦略内）に対する新規エントリーシグナルは無視する（executor レベルで実装済み）
- 同一バーで同一戦略から相反するシグナル（long vs short）が出た場合、いずれも発行しない（矛盾は見送り）。**実装方法**: Strategy trait の `on_price` は 1 シグナルしか返さないため、現状の trend_follow_v1 では構造的に発生しない。将来 1 バーで複数シグナルを返す戦略を追加する場合に engine 側でフィルタリングが必要
- close シグナルと新規 open シグナルが同バーで出た場合、close を先に処理する（ポジションを閉じてから新規を判定）。**実装方法**: SL/TP の close は position-monitor タスクが PriceEvent ごとに先に処理し、新規 open は strategy-engine → executor の順で後から処理されるため、タスク実行順で自然に close 優先になる
...
- **swing_llm_v1**: スイング LLM アシスト（Vegapunk 検索 + Gemini Flash 判断）。on_price 内で gRPC + Gemini API を await。コスト効率のため Gemini Flash を使用（判断頻度が低いスイングトレードに適する）
...
**SL/TP 監視（position-monitor タスク）:**
- PriceEvent を購読し、オープンポジションの SL/TP を tick ごとにチェック
- Long: 現在価格 <= stop_loss → SL ヒット、現在価格 >= take_profit → TP ヒット
- Short: 現在価格 >= stop_loss → SL ヒット、現在価格 <= take_profit → TP ヒット
...
Phase 0 では最小構成:
- **経済指標カレンダー**: 無料で取得可能な API またはフィード（例: Forex Factory HTML パース、investing.com カレンダー API 等）。情報源は実用性で選定し、固定しない
- **ニュース収集**: 無料枠のあるニュース API（例: NewsAPI.org、Alpha Vantage News 等）またはメジャー FX ニュースサイトの RSS フィード。情報源は実用性で選定し、固定しない
- LLM（Gemini Flash）で要約 -> Vegapunk に IngestRaw
- API キーは 1Password + direnv（`op read` パターン）で管理

後回し:
- 戦略パラメータの自動調整
- Trading Economics API（有料、$20/月〜）
...
- **日次バッチタスク**: tokio タスクとして UTC 0:00 に起動し、max_drawdown を計算して daily_summary を更新する。実装は `tokio::time::interval` で毎分チェックし、日付変更時に実行する方式でよい
